...
static bool
cleanup_and_quit_loop (const char * msg, enum AppState state)
{
  if (msg)
    gst_printerr ("%s\n", msg);
  if (state > 0)
...
send_ice_candidate_message (GstElement * webrtc, guint mlineindex,
    char * candidate, gpointer user_data)
{
  char *text;
  JsonObject *ice, *msg;
...
on_negotiation_needed (GstElement * element, gpointer user_data)
{
  ...
  if (remote_is_offerer || our_id) {
    char *msg = g_strdup_printf ("OFFER_REQUEST");
...
static void
data_channel_on_message_string (GObject * dc, char * str, gpointer user_data)
{
...
check_plugins (void)
{
  ...
  const char *needed[] = { "opus", "vpx", "nice", "webrtc", "dtls", "srtp",
    "rtpmanager", "videotestsrc", "audiotestsrc", NULL
  };

  registry = gst_registry_get ();
  ret = true;
  for (i = 0; i < g_strv_length (const_cast<char**>(needed)); i++) {
